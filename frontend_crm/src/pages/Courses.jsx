...
import { Badge, Button, FilterTabs, PageHeader, fmtUZS } from '@/components/ui'
    ...
    <div className="p-8 max-w-[1200px] animate-fade-in">
      ...
      <div className="mb-6">
        <FilterTabs value={filter} onChange={setFilter}
          options={[{ value: 'all', label: 'All' }, { value: 'active', label: 'Active' }, { value: 'upcoming', label: 'Upcoming' }]} />
      </div>
          ...
          <div key={c.id}
            ...
            <div className="p-5">
              ...
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="text-[10px] font-mono text-txt-dim uppercase tracking-widest mb-1.5">{c.tag}</div>
                  <h3 className="font-semibold text-base leading-tight">{c.title}</h3>
                  <div className="text-xs text-txt-muted mt-1">{c.instructor}</div>
                </div>
              ...
              <div className="grid grid-cols-3 gap-3 pt-4 border-t border-border">
                <div>
                  ...
                  <div className="text-xl font-bold" style={{ color: c.color }}>{c.students}</div>
                </div>
                <div>
                  <div className="flex items-center gap-1 text-[10px] font-mono text-txt-dim uppercase mb-1">
                    <Clock size={9} /> Duration
                  </div>
                  <div className="text-xs font-medium text-txt mt-1">{c.duration}</div>
                </div>
                <div>
                  <div className="flex items-center gap-1 text-[10px] font-mono text-txt-dim uppercase mb-1">
                  ...
                  <div className="text-[11px] font-mono text-txt-muted mt-1 leading-tight">
                    {(c.price / 1_000_000).toFixed(1)}M UZS
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="flex justify-between text-[10px] font-mono text-txt-dim mb-1.5">
                  <span>Capacity</span>
                  <span>{c.students}/30</span>
                </div>
                <div className="h-1 bg-bg-4 rounded-full overflow-hidden">
                  <div className="h-full rounded-full transition-all duration-500"
                    style={{ width: `${(c.students / 30) * 100}%`, background: c.color }} />
                </div>
              </div>
            </div>
          </div>
